# Group B1 (finer lrd-weight sweep) of the 2026-08-01 plan -- see
# project_trenberth_lw_transmissivity_gradscale_fix memory, "CORRECTION 2026-08-01".
#
# IMPORTANT CORRECTION that motivates this script: the original reweighting test
# compared each weight's *self-reported* best_smoothed_loss across DIFFERENT loss
# weight functions -- invalid, since a larger lrd weight mechanically changes the
# scale of its own contribution. Rescoring each candidate's actual raw biases
# under one FIXED common yardstick (the original 0.3-weighted TRENBERTH_LOSS)
# gave a different, corrected ranking:
#
#   lrd weight    self-reported loss    score under FIXED 0.3-weighting
#      0.3              148.8                    153.7   (production baseline)
#      0.5              181.9                    121.1   ACTUALLY BEST tested so far
#      1.0              496.9                    255.4   still worst
#
# This was then confirmed at TRUE 7-year climate equilibrium too (not just the
# training proxy, examples/trenberth_equilibrium_crosscheck.jl): weight=0.5 scores
# 504.3 vs weight=0.3's 535.6 under the same fixed yardstick -- a real, validated
# improvement, not a training-metric artifact.
#
# This script (weight=0.4) and its sibling trenberth_bd2_lrdweight06.jl (weight=0.6)
# bracket 0.5 on either side to find the true optimum rather than just knowing 0.5
# beats 0.3 and 1.0. Both launched in parallel (independent runs, no shared state).
# Rank all candidates (0.3/0.4/0.5/0.6/1.0) under the FIXED 0.3-weighting once both
# complete -- never compare self-reported losses across different weights again.
...
# TRENBERTH_LOSS with lrd's weight raised 0.3 -> 0.4 (brackets the confirmed-good
# 0.5 from below). Everything else identical to TRENBERTH_LOSS.
...
result = calibrate!(
    ...
    save_dir = joinpath(@__DIR__, "output/trenberth_bd2_lrdweight04"),
)
