...
#
# V2 CORRECTION (2026-07-30, after v1 result): v1 kept samples_per_batch=32
# fixed while tripling batch_days. `calibrate!`'s `steps_per_sample =
# batch_steps ÷ samples_per_batch` then became 1080÷32=33, and
# gcd(33, steps_per_day=36) = 3 -- only 12 of the 36 daily 40-min time slots
# ever get sampled (repeating every 3rd slot), reintroducing exactly the
# diurnal-cycle aliasing pathology trenberth_full.ipynb's own cell 12 comment
# warns about for a different (samples_per_batch=20) case. v1's result (see
# memory: early-stopped at batch 31, best batch was batch 1, osr got
# substantially worse too) is confounded by this and does NOT cleanly test
# the timescale-mismatch hypothesis -- it tested "batch_days=30 + aliased
# sampling" against "batch_days=10 + clean sampling".
#
# Fix: samples_per_batch=98 keeps steps_per_sample = 1080÷98 = 11, bit-for-bit
# identical to the baseline's 360÷32=11 (same gcd(11,36)=1, same ~7.3h
# sampling interval, same full diurnal coverage) -- this isolates batch_days
# as the only varied factor. max_batches cut to 25 (not 100): AD reverse
# passes, not the plain timesteps, dominate wall-time, and 98 samples/batch
# is ~3x v1's 32, so 25 batches here is already ~3x v1's total AD-pass count.
...
result = calibrate!(
    ...
    TrainingConfig(
        spinup_days       = 180,
        batch_days        = 30.0,
        samples_per_batch = 98,
        max_batches       = 25,   # patience=30 (default) can't fire before max_batches=25 is hit,
                                   # so this guarantees the full 25-batch trajectory regardless of
                                   # early improvement/non-improvement (v1 early-stopped at batch 31).
        grad_clip         = 5f0,
        trunc             = 31,
        nlayers           = 8,
        loss_threshold    = 1f-6,
    ...
    save_dir = joinpath(@__DIR__, "output/trenberth_batchdays30_sensitivity_v2"),
)
