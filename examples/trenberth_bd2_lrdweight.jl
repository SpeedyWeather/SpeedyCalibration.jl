...
#
# This script tests the natural next fix directly: keep batch_days=2 (now
# confirmed as the best window), but reweight TRENBERTH_LOSS so lrd can no
# longer be cheaply traded away. lrd's weight goes from 0.3 -> 1.0 (matching
# osr/olr, the two currently-highest-weighted fluxes) -- a single, minimal,
# surgical change to isolate its effect, everything else (targets, other
# weights, param_specs, batch_days, samples_per_batch) held fixed for a clean
# A/B against the just-completed extended bd=2 run.
#
...
# TRENBERTH_LOSS with lrd's weight raised 0.3 -> 1.0 (matching osr/olr).
# Everything else (flux_keys, targets, other weights) identical to TRENBERTH_LOSS.
...
result = calibrate!(
    ...
    save_dir = joinpath(@__DIR__, "output/trenberth_bd2_lrdweight"),
)
