# Reproduces trenberth_full.ipynb's Sections 6-8 (inspect results, plot training
# history, climate validation) against the real production result now saved at
# output/trenberth_full_result.jld2 (batch_days=2, best_smoothed_loss=148.8).
# Run standalone since no jupyter/nbconvert is available in this environment to
# execute the notebook directly -- this produces the actual output the notebook
# cells would show, so the plots/numbers on disk are real, not stale/fabricated.
#
# Uses the now-fixed run_climate_validation (src/validation.jl: trained_dict was
# result.final_params, now result.best_params -- final_params is the post-drift
# end state and would validate the wrong checkpoint for this run).
