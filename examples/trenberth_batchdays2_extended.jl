# batch_days=2 EXTENDED run -- see project_trenberth_lw_transmissivity_gradscale_fix
# memory for full history. Five prior data points establish a clean,
...
#     grace period, best_smoothed_loss=348.8.
#   batch_days=2  (220-batch budget, 2702s~45min -- cheaper than bd=3, not
#     more expensive): NEVER REVERSED. stop_reason="max batches reached",
#     best_smoothed_loss=238.4 == final loss (still falling on every single
#     batch through the whole 220-batch budget, patience never fired). At
#     batch 220, 5 of 6 fluxes are excellent (osr bias -3.0, srd -2.2, olr
#     +1.6, lru -0.1) -- lrd (bias +25.2) is now ~80% of the entire
#     remaining weighted loss, cleanly isolated as *the* one problem left
#     (same monotonic-growth mechanism tracked since the first analysis of
#     the bd=10 baseline).
#
# This script just re-runs bd=2 with a much bigger max_batches to find where
# (or whether) it actually reverses and what its true floor is -- the 220-
# batch test above simply ran out of budget before any reversal happened, so
# we don't yet know its real grace period or best-achievable loss.
#
...
#
# max_batches=400 (~4x the previous test's budget): at ~12.3s/batch observed
# previously, 400 batches costs ~82 min, comfortable given the previous
# 220-batch run cost only 45 min. Gives 180 batches of extra headroom past
# where the previous run ran out of budget, well beyond bd=3's own grace
# period (131) as a sanity floor -- expectation is bd=2 (the same or better
# on every axis so far) reverses somewhere past batch 220, if it reverses at
# all in this budget.
...
result = calibrate!(
    ...
    save_dir = joinpath(@__DIR__, "output/trenberth_batchdays2_extended"),
)
